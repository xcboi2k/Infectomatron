...
public class SoldierScript : MonoBehaviour
{
    ...
    public AudioSource soldierAudioSource;
    ...
    private void AimandShoot(){
        ...
        if(aim.magnitude > 0.0f){
            ...
            if(Input.GetButtonDown("Jump")){ //Fire1, Jump
                ...
                soldierAudioSource.PlayOneShot(fireClip);
    ...
    void OnTriggerEnter2D(Collider2D target) {
        if(isImmune == false){ //30% damage
            if (target.tag == "Enemy5") {
            GameObject.Find("Gameplay Controller").GetComponent<HealthScript>().currentHealth -= 3.25f;
            Debug.Log("You collided with an Enemy. Decrease health by 5 points");
            soldierAudioSource.PlayOneShot(hurtClip);

            }

            else if (target.tag == "Enemy10") {
            GameObject.Find("Gameplay Controller").GetComponent<HealthScript>().currentHealth -= 6.5f;
