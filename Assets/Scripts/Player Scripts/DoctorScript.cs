...
public class DoctorScript : MonoBehaviour
{
    ...
    private void AimandShoot(){
        ...
        if(aim.magnitude > 0.0f){
            ...
            if(Input.GetButtonDown("Jump")){ //Fire1, Jump
                ...
                doctorAudioSource.PlayOneShot(fireClip);
    ...
    void OnTriggerEnter2D(Collider2D target) {
        if(isImmune == false){ //70% damage
            if (target.tag == "Enemy5") {
            GameObject.Find("Gameplay Controller").GetComponent<HealthScript>().currentHealth -= 1.25f;
            Debug.Log("You collided with an Enemy. Decrease health by 5 points");
            doctorAudioSource.PlayOneShot(hurtClip);
            }

            else if (target.tag == "Enemy10") {
            GameObject.Find("Gameplay Controller").GetComponent<HealthScript>().currentHealth -= 2.5f;
